...
def build_citation_graph(
    ...
    """
    Build a multi-hop citation graph via OpenAlex.

    Parameters
    ----------
    seed_refs : list of {"doi": str|None, "title": str|None}
        One entry per seed reference.
    ...
    # ── Loop 1: Hop-1 = papers that cite any seed paper ─────────────────
    hop1_works: list[tuple[str, dict]] = []
    print(f"[OpenAlex] Loop 1: fetching papers that cite {len(hop0_works)} seed papers …")
    for seed_doi, seed_work in hop0_works:
        cited_by_url = _cited_by_url_for_work(seed_work)
        if not cited_by_url:
            continue
    ...
    print(f"[OpenAlex] Loop 1 done — {len(hop1_works)} hop-1 nodes (papers that cite seeds)")

    hop1_dois = {doi for doi, _ in hop1_works}

    # ── Loop 2: Hop-2 = co-cited references of hop-1 papers; keep top 30 ─
    # Count co-citation frequencies in memory from referenced_works
    # already returned in Loop 1 response (no extra API calls needed).
    cocitation_counts: Counter[str] = Counter()
    for _hop1_doi, work in hop1_works:
        for ref_id in work.get("referenced_works") or []:
            if ref_id:
                cocitation_counts[ref_id] += 1

    # Remove OA IDs known to be in hop-0 or hop-1
    for oa_id in list(cocitation_counts.keys()):
        mapped_doi = oa_id_to_doi.get(oa_id)
        if mapped_doi and (mapped_doi in seed_dois or mapped_doi in hop1_dois):
            del cocitation_counts[oa_id]

    TOP_N_HOP2 = 30
    # Only fetch metadata for the top 30 — 1 batch call instead of hundreds
    top30_ids = [oa_id for oa_id, _ in cocitation_counts.most_common(TOP_N_HOP2)]
    top30_works = _batch_fetch_by_openalex_ids(top30_ids) if top30_ids else []

    top30_dois: list[str] = []
    top30_counts: dict[str, int] = {}
    for work in top30_works:
        paper_doi = _extract_doi(work)
        if not paper_doi or paper_doi in seed_dois or paper_doi in hop1_dois:
            continue
        oa_id = work.get("id") or ""
        cnt = cocitation_counts.get(oa_id, 0)
        if oa_id:
            oa_id_to_doi[oa_id] = paper_doi
        if paper_doi not in graph:
            graph[paper_doi] = _make_node(work, hop=2, cited_by_hop1_count=cnt)
            top30_dois.append(paper_doi)
            top30_counts[paper_doi] = cnt

    sorted_hop2 = sorted(top30_counts.items(), key=lambda x: -x[1])
    print(f"[OpenAlex] Loop 2 done — top {len(top30_dois)} hop-2 articles (co-cited refs of hop-1)")
    if sorted_hop2:
        print(f"[OpenAlex] Hop-2 co-citation counts — min: {sorted_hop2[-1][1]}, max: {sorted_hop2[0][1]}")
...
def load_or_build_citation_graph(
    ...
    if cache_path.exists():
        ...
        if hop0_n > 0 and hop1_n == 0:
            ...
            graph = build_citation_graph(seed_refs)
